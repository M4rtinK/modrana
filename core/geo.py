...
# Advanced geographic calculations and conversions, mostly used
# (at least for now) by the OSM data -> vector tiles conversion logic
# and based/copy-pasted from twms code
    ...
    def transform(self, pr1, pr2, c1, c2):
        if pr1.pstring == pr2.pstring:
            ...
            raise NotImplementedError(
                "Pyproj is not installed - can't convert between projections. Install pyproj please."
            )
projs = {
    "EPSG:4326": {
        "proj":
...
def tile_by_coords(xxx_todo_changeme, zoom, srs="EPSG:3857"):
    ...
    point = [point[0] - projected_bounds[0], point[1] - projected_bounds[1]]
        # shifting (0,0)
    max_p = [projected_bounds[2] - projected_bounds[0],
            projected_bounds[3] - projected_bounds[1]]
    point = [1. * point[0] / max_p[0], 1. * point[1] / max_p[1]]
    # normalizing
