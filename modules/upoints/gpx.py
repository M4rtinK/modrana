...
#adding aditional tags: like the "ele" tag, search comments for ADDHERE
...
class _GpxElem(point.Point):
    ...
    __slots__ = ('name', 'description', 'ele') # ADDHERE

    _elem_name = None

    def __init__(self, latitude, longitude, name=None, description=None, elevation=None): # ADDHERE
        ...
        self.elevation = elevation # ADDHERE
    ...
    def __str__(self, mode="dms"):
        ...
        if self.elevation: # ADDHERE
            text.append("[%d]" % self.elevation) # ADDHERE
        return " ".join(text)
    ...
    def togpx(self, gpx_version=DEF_GPX_VERSION, human_namespace=False):
        ...
        if self.ele: # ADDHERE
            eletag = create_elem("ele", None, gpx_version, human_namespace) # ADDHERE
            eletag.text = self.elevation # ADDHERE
            waypoint.append(eletag) # ADDHERE
...
class Trackpoint(_GpxElem):
    ...
    __slots__ = ('name', 'description', 'elevation') # ADDHERE
        ...
        for version, namespace in accepted_gpx.items():
            ...
            for segment in data.findall(segment_elem):
                ...
                for trackpoint in segment.findall(trackpoint_elem):
                    ...
                    elevation = trackpoint.findtext(ele_elem) # ADDHERE
                    points.append(Trackpoint(latitude, longitude, name,
                                             description, elevation)) # ADDHERE
                self.append(points)
