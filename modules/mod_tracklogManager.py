...
class TracklogManager(RanaModule):
    ...
    def handleMessage(self, message, messageType, args):
        if message in ["up", "down", "reset"]:
            ...
            if m:
                m.setupChooseDistColorMenu('tracklogTools',
                                           '|showTracklogs:colorFromRegister|tracklogManager:setupToolsSubmenu|set:menu:tracklogTools')
    ...
    def drawMenu(self, cr, menuName, args=None):
        ...
        if menuName == 'tracklogManagerCategories':
            ...
            if track.trackpointsList and track.trackpointsList[0]:
                ...
                action3 = "mapView:recentre %f %f|set:showTrackFilename:%s|showTracklogs:makeVisible|set:menu:None" % (
                lat, lon, track.filename)
                menus.drawButton(cr, x3, y3, dx, dy, "show on map", "generic", action3)
            else:
                menus.drawButton(cr, x3, y3, dx, dy, "can't show on map#no points", "generic",
            ...
            if track.elevation == True:
                ...
                #          self.log.info(track.routeInfo['firstElevation'])
                    ...
                    text += "\nelevation of the first point: %s" % units.m2CurrentUnitString(
                        track.routeInfo['firstElevation'], 2, False)
                    text += "\nelevation of the last point: %s" % units.m2CurrentUnitString(
                        track.routeInfo['lastElevation'], 2, False)
    ...
    def setupToolsSubmenu(self):
        ...
        menus.addItem('tracklogTools', 'elevation#get', 'generic',
                      'tracklogManager:getElevation|set:menu:tracklogManager#tracklogInfo')
        menus.addItem('tracklogTools', 'active#set', 'generic',
                      'set:currentTrack:%s|tracklogManager:loadTrackProfile|set:menu:None' % track.filename)
        menus.addItem('tracklogTools', 'inactive#set', 'generic',
                      'set:currentTrack:None|tracklogManager:unLoadTrackProfile|set:menu:None')
        if isVisible:
            menus.addItem('tracklogTools', 'toggle#visible', 'generic',
                          'set:showTrackFilename:%s|showTracklogs:toggleVisible|tracklogManager:setupToolsSubmenu' % track.filename)
        else:
            menus.addItem('tracklogTools', 'toggle#invisible', 'generic',
                          'set:showTrackFilename:%s|showTracklogs:toggleVisible|tracklogManager:setupToolsSubmenu' % track.filename)
        menus.addItem('tracklogTools', 'visible#all tracks', 'generic',
                      'showTracklogs:allVisible|set:menu:tracklogManager#tracklogInfo')
        menus.addItem('tracklogTools', 'visible#no tracks', 'generic',
                      'showTracklogs:clearVisible|set:menu:tracklogManager#tracklogInfo')

        if isVisible:
            colorName = visibleTracklogs[currentPath]['colorName']
            menus.addItem('tracklogTools', 'change color#%s' % colorName, 'generic',
                          'tracklogManager:setupColorMenu|set:menu:chooseDistColor')


    def describeTracklog(self, item, category):
        # describe a tracklog item
        action = ""
        action += "set:activeTracklogPath:%s|loadTracklogs:loadActive|set:menu:tracklogManager#tracklogInfo" % item[
            'path']
        name = item['filename']
